...
- Color: surfaces (the app background, the card paper, raised panels), ink (primary text, and muted text split by surface into an ink-on-dark and an ink-on-light value, because the same muted grey cannot meet contrast on both a dark and a light surface), the accent, and status colors covering both the built-in step and gate states (done, draft, locked, danger) and the consumer-supplied run-status tones the shell already styles (accept, revise, complete), each with its text and background color where it has both.
...
The portals stay mounted on `document.body`. Reparenting them under `.pf-root` is rejected, because today's body mount keeps the fixed overlay full-screen and above everything even when a consumer wraps `ProcessRolodex` in a container that establishes a fixed-positioning containing block or a stacking context (a `transform`, `filter`, `contain`, `perspective`, `will-change`, or a z-indexed positioned wrapper). A fixed overlay nested under `.pf-root` would be bounded by such an ancestor or sit below sibling app UI, so it is not enough that `.pf-root` itself lacks those properties: the risky ancestor is in the consumer's tree. (`.pf-root`'s own deck sets `perspective: 1400px`, a fixed-positioning containing block, which is one reason the overlay is portaled out to body today.)

Because a body-mounted node inherits custom properties only from `html` and `body`, the portal root carries the token scope itself: the element sqnce portals to body declares the same private `--sqnce-_*` block (each resolving `var(--sqnce-public, <default>)`), so the overlays always render with the accessible default tokens wherever they mount. To make consumer overrides reach the portaled overlays as well as the in-flow shell, the implementation propagates `.pf-root`'s effective `--sqnce-*` values onto the portal root (for example by mirroring the resolved token values onto the portal container at render time, or by reading them from a shared theme scope). The plan picks the exact propagation mechanism; the requirement is that the overlays keep their current full-screen, escape-the-app behavior, render with accessible defaults, and pick up consumer overrides. The demo build, including an embedded case with a transformed wrapper, is where to confirm this. See `docs/spikes/80-theming-design-tokens.md` for the fixture characterizing token inheritance and the containing-block behavior.

### 4. Accessibility-safe defaults

The issue requires the default theme to be accessible, independent of any consumer override:

- Meaning never rests on color or a tint alone: every status carries a word and an icon, not a bare colored dot. Audit the places where status is shown only by color. The stage rail pairs a glyph and the stage name for each stage, and done and locked stages get a distinct glyph (a check and a lock); but the current (active) stage, when it shows its number, is distinguished from a not-yet-done reachable stage only by the gold active color, so the active state itself rests on color alone. The step rows show a text state ("Done", "Draft") next to the dot, but the draft and open dots themselves are color-only; the navigation pips are color-only. Where meaning currently rests on color alone and no adjacent word or distinct glyph already conveys the state (the active stage in the rail, the draft and open dots, and the navigation pips), add a visible cue (a glyph, a shape, a ring, or a text label, never color alone); an aria-label is added on top for assistive technology but does not by itself satisfy this, because a sighted color-blind user cannot perceive it. Where an adjacent word or distinct glyph already carries the state, as the step rows do for done and draft and as the rail does for done (check) and locked (lock) stages, that visible cue suffices.
- Measured contrast minimums for body and large text, each color against the surface it actually renders on (target WCAG AA: 4.5:1 for body, 3:1 for large text). The shell has both a dark chrome surface and light card and reading surfaces, so a color is measured against its own background, not one global surface. The implementation audits every default text color against its surface and fixes each that fails; this is not a fixed list decided up front. The spike records the failures found so far (for example `#5E6772` labels at 2.56:1 on dark chrome; `#8A8E96` gate state at 4.46:1 on dark chrome and step state at 3.09:1 on the light card; `#6B6F76` reading status at 4.35:1 on the reading panel; `#9A9EA6` empty-file and JSON-meta text at about 2.5 to 2.7:1 on the card and white renderer surfaces; the required-field asterisk `#C9542D` at 4.12:1 on the card; see `docs/spikes/80-theming-design-tokens.md`), and the implementation re-runs the audit to catch any not listed here. Accessibility takes precedence over byte-identical preservation: the text colors that fail are nudged just enough to pass on their surface, and those contrast fixes are the only intentional changes to the default look. Because `#8A8E96` and `#5E6772` each appear on both a dark and a light surface, where a single nudge cannot pass both, the muted-ink token splits into a dark-surface and a light-surface value rather than one shared token.
...
- Default rendering is visually unchanged from today, apart from the text-color adjustments the contrast audit requires (every default text color that fails AA on its surface, not a fixed list set up front); those contrast fixes are the only intentional visual changes, and accessibility takes precedence over byte-identical preservation.
