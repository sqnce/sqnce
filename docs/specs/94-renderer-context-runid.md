...
1. **Editing / rolodex view** (`ProcessRolodex.jsx`): add the active run id to the `OutputView` context. Note that at the `OutputView` site the name `entry` is shadowed by a per-step `const entry = getStepEntry(run, step.id)` (a `StepEntry`, which has no `id`), so the active run entry is not directly reachable there. Derive the active run id once near the top of the component (where the active run entry is unambiguous) into a dedicated variable, for example `const activeRunId = entry ? entry.id : null;`, and pass `runId: activeRunId`. The nullable form matches how the existing `renderRunHeader` and `runStatus` props already type `runId` (the active entry can be absent), and is the same value already passed to `ReadingView` (`entry ? entry.id : null`); reuse `activeRunId` for that prop too so the active run id has a single source.
