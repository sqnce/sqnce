...
- Gate generation on load completion. `generate()` returns early when `!loaded`, so a click during a still-pending `persistence.load()` does not flush the initial placeholder store over the saved runs. `loaded` initializes to `!persistence`, so this never blocks the no-persistence case; it only blocks the brief window before a present persistence hydrates.
