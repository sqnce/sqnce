...
`frontier` keeps its exact meaning: the furthest committed **spine** stage. For a linear definition the spine is the whole process, so `frontier` ranges over every stage exactly as today and **neither new field ever appears** (a linear run is byte-identical to a current run). For a forked definition, `frontier` tops out at the last spine stage; `trackFrontier` appears the moment the fork opens. Every read of `frontier` for a forked definition is **clamped to the last spine index** (it cannot legitimately exceed it), so a stale persisted linear run whose `frontier` pointed deeper, because the definition later gained a fork, is normalized to "spine committed, fork openable" rather than stranding navigation, fork-opening, or completion; per the pre-publish no-migration stance, that clamp is the only concession and deeper recovery of such a run is not promised.
...
For a forked definition, validators are evaluated against a **track-scoped run**, so cross-track isolation holds for gating, completion, and status, not only draft context. When the engine evaluates a step's validators it passes a sanitized run scoped to the reachable set (spine plus the step's own track), built as an **allowlist** so a future field cannot silently leak: `stepState`, `skips`, and `forces` filtered to that set; `trackFrontier` limited to the current track; the spine `frontier` preserved; `idx` normalized to the validated step's sub-stage; `skippedTracks` and any other field omitted. So neither `getStepEntry` nor a direct read of `ctx.run` can reach a sibling track's outputs, progress, skip, force, or navigation state, and a validator's pass/fail or problem message can never depend on or surface sibling-track state. This threads through the **existing** `opts` without a breaking signature change: the flattened topology reaches the gate evaluation via a new optional `opts.subStages` (already on hand in `advance` / `runSummary` / `isRunComplete` / `trackStatus`); `gateProgress(subStage, run, opts)` and `mainGateProgress(mainStage, run, opts)` keep their parameter lists and scope the validator run when `opts.subStages` describes a forked topology, else pass the run through unchanged. For a linear definition the reachable set is the whole run and no scoping occurs, exactly as today: the run-aware #62-64 contract is unchanged for intra-track and spine relation; only the illegitimate cross-track reach is removed. One reachability rule thus closes the non-leak guarantee for draft context and for gating, completion, and status.
...
- **Stale run:** a forked definition loaded against a persisted run whose `frontier` exceeds the last spine index clamps it to the spine (normalized to spine-committed, fork openable), never stranding navigation or completion.
