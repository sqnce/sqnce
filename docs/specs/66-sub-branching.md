...
- **Run-complete:** false until every kept track (required plus any unskipped optional) has reached its terminal and that terminal's gate is met; true when they have; a kept (unskipped) optional track blocks completion until its own terminal is reached and gated, while a skipped optional track is excluded; completion stays false when a terminal's outputs are prefilled but the frontier has not reached that terminal (`stepState` present, `trackFrontier` / `frontier` still earlier); a forced or gate-unmet terminal does not count; a linear run is complete when `frontier` is at the last main stage with its gate met.
