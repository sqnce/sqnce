...
- **Context scoping:** a draft for a step in one track includes the spine and that track, and excludes the sibling track; a validator on a track step sees a sanitized run view (an allowlist: `stepState` / `skips` / `forces` filtered to spine plus its own track, `trackFrontier` for its own track, the spine `frontier`, `idx` normalized to the validated step's sub-stage; `skippedTracks` and other fields omitted), so a validator that inspects a sibling step's output, a sibling track's `trackFrontier` / `skips` / `forces`, `skippedTracks`, or `ctx.run.idx` (even when the real `idx` is in a sibling or skipped track) finds nothing it can use and cannot affect the gate result or its problem message; linear context and the full-run validator view are unchanged.
