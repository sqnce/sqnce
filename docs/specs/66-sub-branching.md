...
- Scoping the validator view per track. Run-aware validators intentionally receive the full `run` (to relate one step to another via `getStepEntry`, per #62-64); a forked definition must not author a validator that reads a sibling track, and the engine does not police this. The owner's stated leak concern (values reaching LLM draft prompts) is fully closed by the track-scoped `buildContext`; validator problem messages feed gate status (`missing`), not prompts. Per-track validator view-scoping would change the documented validator contract and exceeds this issue; revisit only if a consumer needs enforced isolation.
