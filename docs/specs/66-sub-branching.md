...
The **same reachable set** (spine plus the step's own track) scopes the run view passed to **validators**. For a **linear** definition validators receive the full run unchanged, exactly as today. For a **forked** definition every validator (spine or track step) receives a **sanitized** run built as an **allowlisted shape**, not a denylist, so a future run field cannot silently leak: it carries only `stepState`, `skips`, and `forces` filtered to the reachable set; `trackFrontier` limited to the current track; the shared spine `frontier`; and `idx` **normalized** to the validated step's own sub-stage index (never the user's live browsing position). `skippedTracks` and any field not on the allowlist are omitted. So neither `getStepEntry` nor a direct read of `ctx.run` (outputs, progress, skip, force, or navigation state) can reach a sibling track or depend on where the user is browsing, and a validator's pass/fail decision or problem message can never depend on or surface sibling-track or navigation state. A spine step's validator sees spine only. The run-aware #62-64 contract is unchanged for intra-track and spine relation; only the illegitimate cross-track and navigation reach is removed. One reachability rule thus closes the non-leak guarantee for both draft context and gating, making independent per-track gating structural rather than conventional.
