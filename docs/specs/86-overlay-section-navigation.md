...
The overlay derives the heading outline by parsing the ATX headings from the same markdown source string the renderer receives (the value passed to the renderer), producing a list of entries with text, level, and slug. The parser must mirror the renderer's block handling so it sees exactly the headings the renderer stamps, in the same order. In particular it tracks fenced code blocks (from an opening ` ``` ` line to the next ` ``` ` line) and ignores any heading-looking line inside them, and it recognizes a heading with the same rule the renderer uses (one to six `#` characters at the very start of the line, followed by a space). This matters because the renderer consumes a fenced code block as code before it matches headings, so a `## Usage` line inside a code fence is not a heading and gets no id. A naive parser that scanned every `#` line would add a phantom outline entry pointing at a missing id and, worse, advance the shared slug sequence out of step with the renderer, desynchronizing every later anchor. Keeping this block-handling logic in the shared module (used by both the renderer when it stamps ids and the parser when it builds targets) is what stops the two from drifting. The entry text is plain text for display; if a heading carries inline markdown (for example `## The **bold** title`), the label shows the plain text, while the slug is computed from the same raw heading text the renderer slugifies, so the two still agree. It walks the headings in the same document order through the same slug sequence, so the slug it computes for the nth heading is exactly the id the renderer stamped on that heading. It renders this list as a jump-list pane beside the body; clicking an entry scrolls the overlay body to the matching heading.

The scroll lookup must be scoped to the overlay's own container, not a global `document.getElementById`. When the overlay is open the inline (non-overlay) render is still mounted with the same markdown value, so the same heading ids exist twice in the document (once inline, once in the portaled overlay) and a global id lookup would scroll the inline copy behind the overlay. Resolve the target within the overlay container instead (a ref map from slug to the heading node, or a container-scoped query by the escaped id), then `scrollIntoView` within the overlay's scroll container.
...
The pane renders only when the resolved renderer is the markdown built-in component (the gate is the renderer identity, `Renderer === BUILTIN_RENDERERS.markdown`, not the `render.kind` string) and the document has at least two headings. Keying on the resolved component matters because a consumer can inject a custom renderer for the `markdown` kind, and that override does not stamp the heading ids, so an outline built for it would point at ids that do not exist. Otherwise the overlay is exactly as today (title and Close only): custom renderers for the markdown kind, other built-ins, and non-markdown outputs all keep the current overlay.
